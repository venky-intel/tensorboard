...
class MemoryUtilPluginTest(tf.test.TestCase):
  ...
  def setUp(self):
    ...
    self.plugin = text_plugin.MemoryUtilPlugin(context)
  ...
  def testPluginIsActiveWhenNoRuns(self):
    ...
    multiplexer = event_multiplexer.EventMultiplexer()
    context = base_plugin.TBContext(logdir=self.logdir, multiplexer=multiplexer)
    plugin = text_plugin.MemoryUtilPlugin(context)
...
class MemoryUtilPluginBackwardsCompatibilityTest(tf.test.TestCase):
