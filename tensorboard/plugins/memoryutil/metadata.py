...
from tensorboard.plugins.memoryutil import plugin_data_pb2


PLUGIN_NAME = 'memoryutil'

# The most recent value for the `version` field of the
# `MemoryUtilPluginData` proto.
PROTO_VERSION = 0


def create_summary_metadata(display_name, description):
  """Create a `tf.SummaryMetadata` proto for memoryutil plugin data.
  Returns:
    A `tf.SummaryMetadata` protobuf object.
  """
  content = plugin_data_pb2.MemoryUtilPluginData(version=PROTO_VERSION)
...
def parse_plugin_metadata(content):
  ...
  Arguments:
    content: The `content` field of a `SummaryMetadata` proto corresponding to
      the text plugin.
  Returns:
    A `MemoryUtilPluginData` protobuf object.
  """
  result = plugin_data_pb2.MemoryUtilPluginData()
